...
unsigned int PET_project_compressed_gpu(nifti_image *activityImage, nifti_image *attenuationImage, float *projection, 
               ...
               unsigned int N_samples, float sample_step, float background, float background_attenuation, unsigned int truncate_negative_values, unsigned int direction, unsigned int block_size)
{
    ...
    for (unsigned int index_axial=0; index_axial<N_axial; index_axial++) { 
       for (unsigned int index_azim=0; index_azim<N_azimuthal; index_azim++) { 
          ...
          if (is_active) {
            ...
            if (direction==1)
                et_create_rotation_matrix(  affineTransformation, 
                                            0.5*PI + angles_azimuthal[index_azim],
                                            0,
                                            0.5*PI + angles_axial[index_axial], 
                                            0.0, 
                                            0.0, 
                                            0.0, 
                                            XZY_ROTATION);
                ...
                et_create_rotation_matrix(  affineTransformation, 
                                            angles_azimuthal[index_azim],
                                            0.5*PI, 
                                            0.5*PI + angles_axial[index_axial],   // should it be -0.5*PI+.. 
                                            0.0, 
                                            0.0, 
                                            0.0, 
                                            YXZ_ROTATION);          
...
//fprintf(stdout, "ROTATION:  index_azim: %d   index_axial: %d   \n", index_azim, index_axial); 
//fprintf(stdout, "           angle_azim:        %f   angles_axial:        %f \n",0.5*PI + angles_azimuthal[index_azim], angles_axial[index_axial]); 
...
*/
            ...
            fprintf(stderr," --5 locations[0,1,2,3,4,5]: %d %d %d %d %d %d\n", locations[0],locations[1],locations[2],locations[3],locations[4],locations[5]); 
            fprintf(stderr," --5 locations at offset:    %d %d %d %d %d %d\n", locations[3*offset],locations[3*offset+1],locations[3*offset+2],locations[3*offset+3],locations[3*offset+4],locations[3*offset+5]); 
            fprintf(stderr," --5 locations at end:       %d %d %d %d %d %d\n", locations[3*(N_locations-2)], locations[3*(N_locations-2)+1], locations[3*(N_locations-2)+2], locations[3*(N_locations-2)+3], locations[3*(N_locations-2)+4], locations[3*(N_locations-2)+5]);
            */
...
unsigned int PET_project_compressed_cpu(nifti_image *activityImage, nifti_image *attenuationImage, float *projection, 
               int *offsets, unsigned short *locations, unsigned int *active, unsigned int N_locations, unsigned int N_axial, unsigned int N_azimuthal, 
               float *angles_step_axial, float *angles_azimuthal, unsigned int N_u, unsigned int N_v, float size_u, float size_v, 
...
//fprintf(stdout,"N_locations_plane: %d    offset: %d    bin_u: %d    bin_v: %d    bin_w: %d \n",N_locations_plane,offset,bin_u, bin_v, bin_w );                
                ...
                if (bin_u >= N_u || bin_v >= N_v) { 
                    ...
                    if (offset+i >= N_locations) {
                        ...
                        projection[offset+i] = counts[bin_azimuthal*(N_u*N_v*N_axial) + bin_axial*(N_u*N_v) + bin_u*(N_v) + bin_v]; 
                }
            }
        }
    }
