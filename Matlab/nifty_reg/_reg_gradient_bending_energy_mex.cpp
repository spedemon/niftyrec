...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
   ...
   status = reg_array_gradient_bending_energy(cp_gradient_ptr, control_points_ptr, image_size, cp_size, cp_spacing, weight, use_gpu);
   ...
   /* Return */
   if (status != 0)
   	mexErrMsgTxt("Error while computing bending energy gradient wrt control points locations.");
   return;
}
