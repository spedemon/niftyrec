...
int seg_EM::get_size_x()
{
    return this->CurrSizes->xsize;
}
int seg_EM::get_size_y()
{
    return this->CurrSizes->ysize;
}
int seg_EM::get_size_z()
{
    return this->CurrSizes->zsize;
}
int seg_EM::get_n_images()
{
    return this->CurrSizes->usize;
//    return this->CurrSizes->tsize;  //FIXME
}
int seg_EM::get_n_classes()
{
    return this->CurrSizes->numclass;
}

nifti_image * seg_EM::GetInputImage()
{
    return this->inputImage;
}

nifti_image * seg_EM::GetPriorImage()
{
    return this->Priors;
}

nifti_image * seg_EM::GetMask()
{
    return this->Mask;
}

float seg_EM::GetLoglik()
{
    return this->loglik;
}

float * seg_EM::GetMean()
{
    return this->M;
}

float * seg_EM::GetVariance()
{
    return this->V;
}

int seg_EM::SetExpec(SegPrecisionTYPE * expec_data)
{
    if(this->maskImage_status)
    {
        ...
        SegPrecisionTYPE * tempExpec= (SegPrecisionTYPE *) this->Expec;
        ...
        for(int cl=0; cl<this->CurrSizes->numclass;cl++)
        {
            ...
            for (int i=numel; i--; Maskptrtmp++,DataPTR++)
            {
                if(*Maskptrtmp)
                {
                    *tempExpec = *DataPTR;
    ...
    else
    {
        ...
        for(int cl=0; cl<this->CurrSizes->numclass;cl++)
        {
            for (int i=numel; i--; DataPTR++,Expec_PTR++)
                *Expec_PTR = *DataPTR;
        }
    }
    return 0;
...
int seg_EM::Step_EM_Expectation()
{
        Expectation();
        return 0;
}

int seg_EM::Step_EM_Maximization()
{
        Maximization();
        return 0;
}

int seg_EM::Step_EM_MRF()
{
        UpdateMRF();
        return 0;
}

int seg_EM::Step_EM_BiasField()
{
        UpdateBiasField();
        return 0;
}

int seg_EM::Step_EM_PriorWeight()
{
        UpdatePriorWeight();
        return 0;
}
...
int seg_EM::ChangeInputImage(SegPrecisionTYPE* image_data)
{
        memcpy((void*)(this->inputImage->data),(void*)image_data,this->inputImage->nvox*sizeof(SegPrecisionTYPE));
        Normalize_Image(this->inputImage,CurrSizes,this->verbose_level);
        return 0;
}

float seg_EM::GetRegValue()
{
        return this->reg_factor;
}

int seg_EM::SetMean(float *mean)
{
        memcpy((void*)this->M, (void*)mean, this->get_n_images()*this->get_n_classes()*sizeof(float));
        return 0;
}

int seg_EM::SetVariance(float *variance)
{
        memcpy((void*)this->V, (void*)variance, this->get_n_images()*this->get_n_images()*this->get_n_classes()*sizeof(float));
        return 0;
}
