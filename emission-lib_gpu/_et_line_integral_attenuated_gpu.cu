...
void et_line_integral_attenuated_gpu(float *d_activity, float *d_attenuation, float *d_sinogram, int cam, nifti_image *img, float background_activity)
{
	...
	et_line_integral_attenuated_gpu_kernel <<<G1,B1>>> (d_activity, d_attenuation, currentCamPointer, background_activity);
