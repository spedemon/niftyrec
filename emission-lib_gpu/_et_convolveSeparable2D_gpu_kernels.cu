...
__global__ void convolutionRowsKernel(float *d_Dst,float *d_Src,int imageW,int imageH,int pitch,int kernelRadius){
    ...
    for(int i = ROWS_HALO_STEPS; i < ROWS_HALO_STEPS + ROWS_RESULT_STEPS; i++){
        ...
        for(int j = -kernelRadius; j <= kernelRadius; j++)
            sum += c_Kernel[kernelRadius - j] * s_Data[threadIdx.y][threadIdx.x + i * ROWS_BLOCKDIM_X + j];
